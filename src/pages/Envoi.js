...
export default function Ramassage() {
  ...
  const filteredEnvoi = applySortFilter(envoi, getComparator(order, orderBy), filterName, filters);

  const isNotFound = !filteredEnvoi.length && !!filterName;

  // My Modifs ----

  const getEnvoi = async () => {
    try {
      const res = await Request.get(`/envoi/by-client`);
      setEnvoi(res.data);
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                />
                <TableBody>
                  {filteredEnvoi.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, wilaya, commune, address, infos, client, etats, name, prixtotal, produit, quantite } =
                      row;
                      ...
                      wilaya && (
                        <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                          ...
                          <TableCell align="left">
                            <Label color={(status.etat.code !== 'banned' && 'error') || 'success'}>
                              {status.etat.name}
                            </Label>
                          </TableCell>

                          <TableCell align="right">
                            <Link to={`/impression/${_id}`}>
